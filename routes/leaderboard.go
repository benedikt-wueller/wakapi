...
func (h *LeaderboardHandler) buildViewModel(r *http.Request, w http.ResponseWriter) *view.LeaderboardViewModel {
	...
	if byParam == "" {
		leaderboard, err = h.leaderboardService.GetByInterval(models.IntervalPast12Months, pageParams, true)
		if err != nil {
		...
		if user != nil && !leaderboard.HasUser(user.ID) {
			...
			if count, err := h.leaderboardService.CountUsers(); err == nil && count > int64(pageParams.PageSize) {
				if l, err := h.leaderboardService.GetByIntervalAndUser(models.IntervalPast12Months, user.ID, true); err == nil && len(l) > 0 {
		...
		if by, ok := allowedAggregations[byParam]; ok {
			leaderboard, err = h.leaderboardService.GetAggregatedByInterval(models.IntervalPast12Months, &by, pageParams, true)
			if err != nil {
				conf.Log().Request(r).Error("error while fetching general leaderboard items - %v", err)
				return &view.LeaderboardViewModel{
					Messages: view.Messages{Error: criticalError},
			...
			if user != nil {
				...
				if count, err := h.leaderboardService.CountUsers(); err == nil && count > int64(pageParams.PageSize) {
					if l, err := h.leaderboardService.GetAggregatedByIntervalAndUser(models.IntervalPast12Months, user.ID, &by, true); err == nil {
